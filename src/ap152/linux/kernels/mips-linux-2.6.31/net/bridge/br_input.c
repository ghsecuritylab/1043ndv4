...
EXPORT_SYMBOL(athrs_hnat_delete_frag_napt_entry);
#ifdef CONFIG_ATHRS17_HNAT_WIFI
int (*athrs_hnat_wifi_redirect)(struct sk_buff *skb);
EXPORT_SYMBOL(athrs_hnat_wifi_redirect);
int (*athrs_hnat_kernel_ct_restore)(struct sk_buff *skb);
EXPORT_SYMBOL(athrs_hnat_kernel_ct_restore);
#endif
#endif
/*
...
struct sk_buff *br_handle_frame(struct net_bridge_port *p, struct sk_buff *skb)
{
	...
	int (*ct_restore)(struct sk_buff *skb);
#endif
#endif
...
forward:
	...
	case BR_STATE_LEARNING:
...
#ifdef CONFIG_ATHRS17_HNAT_WIFI
		if (!skb->is_hnat_frag && skb->is_wifi_redirect && skb->is_from_eth && (p->state == BR_STATE_FORWARDING)
			&& (p->dev->priv_flags & IFF_HNAT_WIFI_REDIRECT_PORT) && (skb->protocol == htons(ETH_P_IP)) && (skb->len > sizeof(struct iphdr))) {
			ct_restore = rcu_dereference(athrs_hnat_kernel_ct_restore);
			if (ct_restore)
				ct_restore(skb);
		}

		// HNATed packet from wifi to WAN
		...
		// HNATed packet from WAN to WIFI
