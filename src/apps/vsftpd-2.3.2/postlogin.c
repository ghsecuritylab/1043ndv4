...
static int handle_user_permission(struct vsf_session* p_sess);
...
process_post_login(struct vsf_session* p_sess)
{
  ...
  while(1)
  {
    ...
    {
		  /*add by ZQQ, 2013.03.11*/
 	if (-1 == handle_user_permission(p_sess))
 	{
		return;
	}
 		 /* end added by ZQQ, 12Mar13 */
    }
...
/******************************************************************************
* FUNCTION      :	handle_user_permission()
* AUTHOR        :	zhouqiqiu <zhouqiqiu@tp-link.net>
* DESCRIPTION   :	handle the user's permission 
* INPUT         : 	N/A
*
* OUTPUT        : 	N/A
* RETURN        : 	N/A
...
static int handle_user_permission(struct vsf_session* p_sess)
{
		...
		{
			memset(name, 0, 128);
			getPasswdInfo(i, name, &permission);
					
			if ((strlen(name) > 0) && (0 == strcmp(name, str_getbuf(&p_sess->ori_user_str))))
			{
				//sprintf(tmp, "==== %s, %d, %s", name, permission, str_getbuf(&p_sess->ori_user_str));
				//die(tmp);
				if ((str_equal_text(&p_sess->ftp_cmd_str, "STOR") 
					|| str_equal_text(&p_sess->ftp_cmd_str, "DELE")
					|| str_equal_text(&p_sess->ftp_cmd_str, "APPE")
					|| str_equal_text(&p_sess->ftp_cmd_str, "STOU")
					|| str_equal_text(&p_sess->ftp_cmd_str, "MKD")
					...
					|| str_equal_text(&p_sess->ftp_cmd_str, "RNFR") 
					|| str_equal_text(&p_sess->ftp_cmd_str, "RNTO"))
					&& permission == FTP_READ_ONLY)
				{
					output("vsftpd: refusing to run with read only permission, permission denied");
					return -1;
			}
			else
			{
