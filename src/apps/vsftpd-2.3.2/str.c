...
static FTP_PERMISSION ftp_permissions[MAX_USER_NUM]=
{
	{NULL, -1},
	{NULL, -1},
	{NULL, -1},
...
{	
	...
	{		
		...
		/* added by ZQQ, 12Mar13 */
		if (ncounter < MAX_USER_NUM)
		{
			strncpy(ftp_permissions[ncounter].username, name, strlen(name));
			ftp_permissions[ncounter].permission = ftpPermission - 48; /*0 µÄasciiÂëÊÇ48*/
			
			//sprintf(tmp, "echo %s %d username = %s, permission = %d >> /tmp/debug.txt", __FUNCTION__,__LINE__, 
			//	ftp_permissions[ncounter].username,ftp_permissions[ncounter].permission);
			//system(tmp);
			ncounter++;
		}
		else
	...
	{
		sprintf(tmp, "echo %s %d username = %s, permission = %d >> /tmp/debug.txt", __FUNCTION__,__LINE__, 
		ftp_permissions[ncounter].username, 
		ftp_permissions[ncounter].permission);
		system(tmp);
		//die(tmp);
	}
#endif			
...
void getPasswdInfo(int index, char *pName, int *pPermission)
{
	strncpy(pName, ftp_permissions[index].username, sizeof(ftp_permissions[index].username));
	*pPermission = ftp_permissions[index].permission;	
}
