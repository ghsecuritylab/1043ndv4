...
//Do not do resume stuff here, and how it handles is wrong!
/*  
...
****************************************************************************/
BOOL dir_check_ftype(int cnum,int mode,SMB_STRUCT_STAT *st,int dirtype)
{
...
****************************************************************************/
BOOL get_dir_entry(int cnum,char *mask,int dirtype,char *fname,SMB_OFF_T *size,int *mode,time_t *date,BOOL check_descend)
{
  char *dname;
  BOOL found = False;
...
void *OpenDir(int cnum, char *name, BOOL use_veto)
{
  ...
  dirp = (smb_Dir *)malloc(sizeof(smb_Dir));
  if (!dirp) {
    sys_closedir(p);
    return(NULL);
  }
  ...
  {
/*NOTE: file name with upper characters on FAT filesystem is really a pain in ass!!!!
  *it does not work right in this case..... Need to be fixed.....*
*/
    int l = strlen(n)+1;
...
//search the dir to see if we have the file
BOOL SearchDir(int cnum, const char *name)
{
	BOOL res = False;
	smb_Dir *dirp = NULL;
	smb_Dir *tmp = (smb_Dir *)(Connections[cnum].dirptr);
	
	dirp = (smb_Dir *)malloc(sizeof(smb_Dir));
	if(dirp)
		memcpy(dirp, Connections[cnum].dirptr, sizeof(smb_Dir));
	else
		return False;
	...
	while(dirp->pos <= tmp->pos){
		if(case_sensitive ? (strcmp(ReadDirName(dirp), name) == 0) : strequal(ReadDirName(dirp), name))
		{
			if(!IS_VETO_PATH(cnum, name)){
				DEBUG(0,("yes, resume is okay!!!\n"));
				res = True;
				break;
			}
		}
...
../../../../../../../../../../../../../../../../../../../../../../../../../..";
  ...
  size_t dotsize;
  SMB_DEV_T rootdev, thisdev;
  SMB_INO_T rootino, thisino;
