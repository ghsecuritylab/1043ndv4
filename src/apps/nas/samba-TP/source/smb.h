...
#if defined(LARGE_FILE_SUPPORT)
#define BUFFER_SIZE (128*1024)
#else /* no large readwrite possible */
#define BUFFER_SIZE (0xFFFF)
#endif

#define SYNC_AFTER_TRANS 256
...
typedef struct
        {
	ssize_t   wr_total_written; /* So we know when to discard this */
	int32 wr_timeout;
	int32 wr_errclass;
...
typedef struct
{
  ...
  int mode;
  int uid;
/*we do not use mmap anymore for memory save consideration!*/
#if USE_MMAP
  char *mmap_ptr;
  uint32 mmap_size;
#endif
...
typedef struct
{
  ...
  int share_mode;
  struct timeval time;
  //add the following attribute for winNT
  uint32 desired_access;
} share_mode_entry;
...
//never use this.... replace it with smb_roundup() function!
...
//if we have mempool in future, replace these methods!!!
...
#define SMB_ASSERT(b) ( (b) ? (void)0 : (exit(1)))
#define dos_to_unix(x,y) (x)	//it is useless. Do nothing:)
