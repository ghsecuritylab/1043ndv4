...
****************************************************************************/
BOOL is_locked(int fnum,int cnum,SMB_OFF_T count,SMB_OFF_T offset)
{
  int snum = SNUM(cnum);
...
****************************************************************************/
BOOL do_lock(int fnum,int cnum,size_t count,SMB_OFF_T offset,int *eclass,uint32 *ecode)
{
  BOOL ok = False;
  ...
  if (Files[fnum].can_lock && OPEN_FNUM(fnum) && (Files[fnum].cnum == cnum))
    ok = fcntl_lock(Files[fnum].fd_ptr->fd,F_SETLK,offset,(SMB_OFF_T)count,
                    (Files[fnum].can_write?F_WRLCK:F_RDLCK));
...
****************************************************************************/
BOOL do_unlock(int fnum,int cnum,size_t count,SMB_OFF_T offset,int *eclass,uint32 *ecode)
{
  BOOL ok = False;
  ...
  if (Files[fnum].can_lock && OPEN_FNUM(fnum) && (Files[fnum].cnum == cnum))
    ok = fcntl_lock(Files[fnum].fd_ptr->fd,F_SETLK,offset,(SMB_OFF_T)count,F_UNLCK);
