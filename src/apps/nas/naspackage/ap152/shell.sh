...
rm -f ${ROOTFS_DIR}/lib/libntfs-3g.so.83
rm -f ${ROOTFS_DIR}/lib/libntfs-3g.so.83.0.0
cd ${ROOTFS_DIR}/lib/
ln -s libntfs-3g.so.83.0.0 libntfs-3g.so.83
ln -s libntfs-3g.so.83.0.0 libntfs-3g.so
cp -f ${TOP_DIR}/ntfs-3g/libntfs-3g.so.83.0.0 ${ROOTFS_DIR}/lib/libntfs-3g.so.83.0.0
...
#added by ZQQ 2014.11.18
if grep 'mount -t usbfs none /proc/bus/usb' ${ROOTFS_DIR}/etc/rc.d/rcS > /dev/null 2>&1 ; then \
echo "rcS already have mount -t usbfs none /proc/bus/usb"  ;\
else \
sed 's/mount -t ramfs -n none \/var/mount -t ramfs -n none \/var\nmount -t usbfs none \/proc\/bus\/usb/' ${ROOTFS_DIR}/etc/rc.d/rcS > ${ROOTFS_DIR}/etc/rc.d/rcS.tmp && \
mv -f ${ROOTFS_DIR}/etc/rc.d/rcS.tmp ${ROOTFS_DIR}/etc/rc.d/rcS && \
chmod 755 ${ROOTFS_DIR}/etc/rc.d/rcS ; \
fi
#endif 